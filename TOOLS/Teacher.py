...
from TOOLS.loss_function import category_error
...
from TOOLS.loss_function import l1_regularization_columnwise
...
def keep_top_k_values_per_column(input_matrix, k=50):
    values, indices = torch.topk(torch.abs(input_matrix), k, dim=0)  # compute along the column dimension
    threshold = torch.abs(values[k - 1, :])  # use the kth value in each column as the threshold
...
class AdaptiveSoftshrink(nn.Module):
    ...
    def forward(self, x, rho):
        # subtract lambda from the absolute value and apply ReLU
...
class admm_norm(nn.Module):
    ...
    def forward(self, x):
        ...
        norm = torch.norm(x, p=1, dim=-1, keepdim=True)  # L1 norm
        return x / norm
...
class Teacher(nn.Module):
    ...
    def innitialize_c(self, X, rho):
        ...
        # use a Cholesky factorization instead of explicit inversion
        L = torch.linalg.cholesky(m)
        I = torch.eye(m.shape[0], device=m.device)  # identity matrix
        m_1 = torch.cholesky_solve(I, L)  # compute the inverse of m
    ...
    def loss(self, X_p, X_r, H, c_i, H_r, sim, z_i, n_cluster, label_true):
        ...
        # c_loss
        loss_c1 = l1_regularization_columnwise(matrix=c_i)

        # distill from C to classfication
        loss_info = self.contras.contrastive(None, c_i, sim, label_true)
